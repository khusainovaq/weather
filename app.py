...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:123@localhost/weather'
...
def weather():
    ...
    query = db.session.execute(text(f"""SELECT * FROM cities WHERE name = '{name}';"""))
    result = query.fetchone()
    print(result)
    if result is None:
        db.engine.execute(text(f"""INSERT INTO cities (name, count) VALUES ('{name}', 1);"""))
    else:
        db.engine.execute(text(f"""UPDATE cities SET count = count + 1 WHERE name = '{name}';"""))
    db.session.commit()
